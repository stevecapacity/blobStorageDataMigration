...
Start-Transcript -Path "$($localPath)\pre-migration.log" -Verbose
...
Write-Host "Created SMB share $($shareName) at path C:\Users\$($userName)"
...
$filePath = "$($localPath)\$($hostname)-$($userName).xml"
$xmlString | Out-File -FilePath $filePath
$blobNameXML = $hostname + "-" + $userName + ".xml"
Set-AzStorageBlobContent -File $filePath -Container $containerName -Blob $blobNameXML -Context $context -Force
Write-Host "Uploaded XML file to blob storage"
# =========================================================================================

# BACKUP LOCAL USER DATA TO BLOB STORAGE
#loop through each directory in locations array
foreach($location in $locations)
{
    # FOR APPDATA DIRECTORIES, THIS REMOVES SPECIAL CHARACTER "\" FROM BLOB STORAGE
    ...
    {
        # ALL OTHER DIRECTORIES USER THIS METHOD
